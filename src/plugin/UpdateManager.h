...
// Shared Sapp-family settings file (same one all Sapp instruments use).
inline juce::PropertiesFile& sappSharedSettings()
{
...
private:
    ...
    static constexpr const char* kRepo = "localteampartners/sappchoir";
    ...
    void runCheck()
    {
        ...
        auto stream = api.createInputStream(
            juce::URL::InputStreamOptions(juce::URL::ParameterHandling::inAddress)
                .withExtraHeaders("User-Agent: SappChoir-Updater")
        ...
        if (!sapp::choir::isNewerVersion(tag.toRawUTF8(), JucePlugin_VersionString))
    ...
    void runInstall()
    {
        ...
        for (const auto& dir : tempDir.findChildFiles(juce::File::findDirectories, false))
            if (dir.getFileName().startsWith("SappChoir")) { stage = dir; break; }
    ...
    {
        const auto newVst = stage.getChildFile("SappChoir.vst3");
        ...
        if (target == juce::File())
            target = juce::File::getSpecialLocation(juce::File::windowsLocalAppData)
                         .getChildFile("Programs/Common/VST3/SappChoir.vst3");

        const auto parked = target.getSiblingFile("SappChoir.vst3.old-" + tag);
    ...
    bool installMac(const juce::File& stage, const juce::String& tag, juce::String& note)
    {
        ...
        install("SappChoir.vst3", home.getChildFile("Library/Audio/Plug-Ins/VST3"));
        install("SappChoir.component", home.getChildFile("Library/Audio/Plug-Ins/Components"));

        // Standalone: stage the new app next to the current one.
        if (juce::JUCEApplicationBase::isStandaloneApp()) {
            const auto newApp = stage.getChildFile("SappChoir.app");
            const auto currentApp =
                juce::File::getSpecialLocation(juce::File::currentApplicationFile);
            if (newApp.exists() && currentApp.getFileName().endsWith(".app")) {
                const auto parked = currentApp.getSiblingFile("SappChoir.app.old");
                parked.deleteRecursively();
